...
export default {
  ...
  methods: {
    ...
    bookmarkAlbum(album) {
      if (this.isInBookmark(album.m_id)) {
        Swal.fire({
          icon: 'question',
          text: `"${album.m_title}"를(을) 북마크에서 삭제하나요?`,
          ...
          if (res.isConfirmed) {
            ...
            axios
              .delete(
                `${spring_URL}/likemusic/delete?lmMNo=${album.m_no}&lmUNo=${this.uId}`
              )
              .then((res) => {
                if (res.data == '관심 음원 삭제 성공') {
                  this.snackbar = true;
                  this.snackbarColor = 'error';
                  this.snackbarText = `${album.m_title} 북마크에서 제거`;
                }
              })
              .then(() => {
                axios
        ...
        axios
          .post(
            `${spring_URL}/likemusic/add?lmMNo=${album.m_no}&lmUNo=${this.uId}`
          )
          .then((res) => {
            if (res.data == '관심 음원 등록 성공') {
              this.snackbar = true;
              this.snackbarColor = 'success';
              this.snackbarText = `"${album.m_title}" 북마크에 추가`;
            }
          })
          .then(() => {
            axios
